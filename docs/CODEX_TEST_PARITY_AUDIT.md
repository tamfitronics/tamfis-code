...
| Stop (can force the agent to keep working via `{"decision": "block"}`, re-injecting into the same round loop) | `hooks.py`'s `session_completed` + `run_session_completed_hooks()`, `runner_local.py` completion boundary | COVERED (added 2026-09-14) — structured block decisions run before persistence, append a repair instruction, and re-enter the same model loop with a bounded three-block guard. Confirmed live in `tests/test_claude_parity_hooks.py` |
