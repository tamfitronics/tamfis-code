...
def test_openrouter_default_is_not_openai_family():
    ...
    # Automatic routing remains provider-flexible and does not force OpenAI,
    # while economical OpenAI models may be explicitly selected.
    assert not cfg.default_model.startswith("openai/")
    assert "openai/gpt-4.1-mini" in cfg.models
    assert "openai/gpt-4.1-nano" in cfg.models
...
def test_remote_fallback_candidates_stay_in_policy_order():
    ...
    # Automatic fallback excludes HF because it has no declared free route;
    # OpenRouter remains eligible only through its configured :free model.
...
def test_standalone_provider_manager_excludes_tier_iv_from_routing_order():
    ...
    assert manager.fallback_chain_names(ProviderType.NVIDIA) == [ProviderType.OPENROUTER.value]


def test_cross_provider_fallback_is_disabled_by_default(monkeypatch):
    monkeypatch.delenv("TAMFIS_CODE_ALLOW_PROVIDER_FALLBACK", raising=False)
    monkeypatch.delenv("TAMFIS_CODE_DISABLE_PROVIDER_FALLBACK", raising=False)
    manager = ProviderManager.__new__(ProviderManager)
    assert manager.auto_fallback_enabled()
    assert not manager.paid_fallback_enabled()


def test_cross_provider_fallback_requires_explicit_opt_in(monkeypatch):
    monkeypatch.setenv("TAMFIS_CODE_ALLOW_PROVIDER_FALLBACK", "true")
    manager = ProviderManager.__new__(ProviderManager)
    assert manager.auto_fallback_enabled()
    assert manager.paid_fallback_enabled()


def test_all_automatic_fallback_can_be_disabled_explicitly(monkeypatch):
    monkeypatch.setenv("TAMFIS_CODE_DISABLE_PROVIDER_FALLBACK", "true")
    manager = ProviderManager.__new__(ProviderManager)
    assert not manager.auto_fallback_enabled()
