...
class OrchestratorTests(unittest.TestCase):
    ...
    def test_huge_objective_is_not_duplicated_unbounded_into_the_system_prompt(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            # ~5.6k chars; supplemental adds the bounded objective preview,
            ...
            self.assertLess(len(system_message["content"]), 17_000)
    ...
    def test_tool_results_advance_plan_step_status(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            # _advance_plan_step (orchestrator/engine.py) starts and, when the
            # observation is useful, completes a step within the same call --
            # it does not wait for a second observation to close it out. One
            # useful tool result is enough to both start step 0 and complete
            # it, immediately arming step 1 as in_progress.
            self.assertEqual(run.plan.steps[0].status, "completed")
            self.assertEqual(run.plan.steps[1].status, "in_progress")
            self.assertTrue(all(step.status == "pending" for step in run.plan.steps[2:]))
            ...
            self.assertEqual(run.plan.steps[0].status, "completed")
            self.assertEqual(run.plan.steps[1].status, "completed")
            self.assertEqual(run.plan.steps[2].status, "in_progress")
    ...
    def test_fail_marks_in_progress_step_failed(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            envelope.finish(result={"success": True, "result": "ok"}, success=True)
            engine.record_tool(envelope)
            # One useful observation already completes step 0 and arms step 1
            # as in_progress (see test_tool_results_advance_plan_step_status)
            # -- fail() only ever demotes whichever step is in_progress at
            # the moment of failure, so that's step 1 here, not step 0.
            self.assertEqual(run.plan.steps[0].status, "completed")
            self.assertEqual(run.plan.steps[1].status, "in_progress")
            engine.fail("provider unavailable")
            self.assertEqual(run.plan.steps[1].status, "failed")
            self.assertEqual(run.plan.steps[0].status, "completed")
