...
class ProviderManager:
    ...
    def fallback_chain_names(self, current: ProviderType) -> list[str]:
        return [provider.value for provider in self.routing_order if provider != current]
    ...
    def resolve_route(
        ...
        *,
        ...
        resolved = (
            self._select_best_provider(task_profile, quality_mode=quality_mode)
            if provider == ProviderType.AUTO
            else provider
        )
    ...
    async def chat_completion(
        ...
        **kwargs: Any,
        ...
        selected_model = model or self.select_model(config, task_profile)
