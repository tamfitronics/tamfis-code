...
def build_system_prompt(session_id: int, workspace_root: Path, *, force_discovery: bool = False) -> str:
    ...
    lines = [
        ...
        "trust its bounded tree-discovery result: use a unique resolved path it returns, "
        "or choose one exact path from its ambiguity list; do not repeat the same missing "
        "path or conclude the file is absent before checking the reported candidates. "
        "Before checking whether any local service is "
